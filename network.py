...
class SuperTrack:
    def __init__(self, device):
        ...
        self.world_dataloader = BVHdataset(window_size=self.world_window).get_loader(self.world_batchsize)
        print("done")
        print("preparing data...")
        self.policy_dataloader = BVHdataset(window_size=self.policy_window).get_loader(self.policy_batchsize)
        print("done")
    ...
    def forward(self, pos, vel, rot, ang, j_rot, j_ang, type = 'world'):
        ...
        for i in range(self.world_window):
            ...
            s_rot = pyt.matrix_to_quaternion(torch.matmul(pyt.axis_angle_to_matrix(s_ang), pyt.quaternion_to_matrix(s_rot)))
    ...
    def train_world_loss(self, pos1, pos2, vel1, vel2, rot1, rot2, ang1, ang2):
        ...
        loss += wrot*torch.mean(torch.sum(torch.abs(ang1-ang2), dim = -1))
        print(pos1-pos2)
        print(vel1-vel2)
        print(ang1-ang2)
        print(pyt.quaternion_multiply(rot1, pyt.quaternion_invert(rot2)))
        raise Exception
        loss += wang*torch.mean(torch.sum(torch.abs(pyt.quaternion_multiply(rot1, pyt.quaternion_invert(rot2))), dim = -1))
    ...
    def train(self):
        ...
        for epoch in range(self.EPOCH):
            for data1, data2 in zip(self.world_dataloader, self.policy_dataloader):
                pos1 = data1[0].to(self.device).to(torch.float32)
                vel1 = data1[1].to(self.device).to(torch.float32)
                rot1 = data1[2].to(self.device).to(torch.float32)
                ang1 = data1[3].to(self.device).to(torch.float32)
                j_rot1 = data1[4].to(self.device).to(torch.float32)
                j_ang1 = data1[5].to(self.device).to(torch.float32)

                pos2 = data2[0].to(self.device).to(torch.float32)
                vel2 = data2[1].to(self.device).to(torch.float32)
                rot2 = data2[2].to(self.device).to(torch.float32)
                ang2 = data2[3].to(self.device).to(torch.float32)
                j_rot2 = data2[4].to(self.device).to(torch.float32)
                j_ang2 = data2[5].to(self.device).to(torch.float32)
